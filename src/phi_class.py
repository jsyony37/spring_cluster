...
class phi:
    ...
    def __init__(self, hs_file=None, supercell=[1, 1, 1]):
        ...
        self.moddict_cells = (
            {}
        )  # caches information on distances between atoms for various supercells
        ...
        self.exact_constraint = (
            []
        )  # we calculate the energy of these file numbers exactly

        self.weight_by_energy = (
            False  # if true, we use a formula to weight large energy structures less
        )
    ...
    def load_hs(self, hs_file, supercell):
        ...
        TIME = [time.time()]
        Acell, atoms, coords_hs, coords_type, masses, kpts = load_atomic_pos(
            hs_file, return_kpoints=True
        )
        ...
        if self.verbosity == "High":
            ...
            for R, trans in zip(
                self.dataset["rotations"], self.dataset["translations"]
            ):
                print(R)
                print(trans)
                print("-")
    ...
    def load_hs_output(self, hs_file_out):
        ...
        for n in range(np.prod(self.supercell)):
            self.forces_ref[(n * self.nat) : ((n + 1) * self.nat), :] += forces[
                0 : self.nat, :
            ]
        ...
        for x in range(supercell[0]):
            for y in range(supercell[1]):
                for z in range(supercell[2]):
                    ...
                    for col in range(3):
                        coords_super[
                            list(range(c * self.nat, (c + 1) * self.nat)), col
                        ] = (self.coords_hs[:, col] + float(xyz[col])) / supercell[col]
                    for at in range(c * self.nat, (c + 1) * self.nat):
                        supercell_number[at] = [x, y, z]
                        supercell_index[at] = (
                            x * supercell[2] * supercell[1] + y * supercell[2] + z
                        )
    ...
    def set_supercell(self, supercell, nodist=False):
        ...
        self.natsuper = int(self.nat * np.prod(supercell))
        TIME.append(time.time())

        (
            self.Acell_super,
            self.coords_super,
            self.supercell_number,
            self.supercell_index,
        ) = self.generate_cell(supercell)
        ...
        if not nodist:
            dmax, dmax_nosym = self.cutoff_check(
                self.Acell_super, self.coords_super, self.supercell
            )
    ...
    def find_closest_atom_fast(self, cnew, cref=[]):
        ...
        for at in range(natsuper):
            ...
            if (
                (dist[0] < 1e-7 or abs(dist[0] - 1) < 1e-7)
                and (dist[1] < 1e-7 or abs(dist[1] - 1) < 1e-7)
                and (dist[2] < 1e-7 or abs(dist[2] - 1) < 1e-7)
            ):
                dmin = 0.0
                AT = at
                break
    ...
    def make_dist_array_other(self, A, pos, supercell_ref, supercell_index):
        ...
        (
            ...
            pos, A, self.nat, pos.shape[0], supercell_ref, supercell_index
        )
    ...
    def make_dist_array(self):
        ...
        print(type(self.coords_super))
        print(type(self.Acell_super))
        print(type(self.nat))
        print(type(self.natsuper))
        print(type(self.supercell))
        print(type(self.supercell_index))
        (
            self.dist_array,
            self.dist_array_R,
            self.dist_array_R_prim,
            self.dist_array_prim,
            self.moddict,
            self.moddict_prim,
        ) = make_dist_array(
            self.coords_super,
        ...
        for x in range(-1, 2):
            for y in range(-1, 2):
                for z in range(-1, 2):
                    ...
                    if (
                        abs(dist - dmin) < 1e-5
                    ):  # found a copy.  equidistant periodic copy forces cancel harmonically if cell changes.
    ...
    def unfold_to_supercell(
        ...
        if len(cell) == 3:
            ...
            for i in range(3):
                supercell_small[i] = int(
                    round(np.linalg.norm(A[i, :]) / np.linalg.norm(self.Acell[i, :]))
                )
        ...
        forces_new = np.zeros(
            (pos.shape[0] * ncells_needed, 3), dtype=float
        )  # forces are concatenated as well
        natold = pos.shape[0]
        ...
        for x in range(factor[0]):
            for y in range(factor[1]):
                for z in range(factor[2]):
                    ...
                    c += 1
                    for i in range(3):
                        pos_new[c * natold : c * natold + natold, i] = pos[
                            :, i
                        ] / factor[i] + float(xyz[i]) / float(factor[i])
    ...
    def cutoff_check(self, A, pos, supercell_ref):
        ...
        refA, refpos, t1, supercell_index = self.generate_cell(
            supercell_ref
        )  # gen high symm
        dist_array, moddict = self.make_dist_array_other(
            refA, refpos, supercell_ref, supercell_index
        )
        nat = refpos.shape[0]
        dmax = 0.0
        dmax_nosym = 0.0
        ...
        for a in range(self.nat):
            for b in range(nat):
                if abs(dist_array[a, b] - self.firstnn) < 1e-5:
                    ...
                    ss[0] = (
                        ss[0] - moddict[a * nat + b][0][0] * supercell_ref[0]
                    )  # pbcs
                    ss[1] = (
                        ss[1] - moddict[a * nat + b][0][1] * supercell_ref[1]
                    )  # pbcs
                    ss[2] = (
                        ss[2] - moddict[a * nat + b][0][2] * supercell_ref[2]
                    )  # pbcs
    ...
    def calc_polarization(self, u_crys, A=None):
        ...
        if self.useewald == False:
            print(
                "warning cannot calculate polarization without born effective charges"
            )
            return 0.0, 0.0
        ...
        reduced_polarization = np.dot(
            np.linalg.inv(np.eye(3) + strain), physical_polarization
        )
    ...
    def unfold_input(self, fl, use_input=False):
        ...
        A, types, pos, forces, stress, energy, supercell_ref, refA, refpos, bf = (
            self.unfold(A, types, pos, bestfitcell, forces, stress, energy)
        )

        return A, types, pos

    def unfold(
        self, A, types, pos, bestfitcell, forces=None, stress=np.zeros((3, 3)), energy=0
    ):
        ...
        if (
            bestfitcell[0, 1] != 0
            or bestfitcell[0, 2] != 0
            or bestfitcell[1, 2] != 0
            or bestfitcell[1, 0] != 0
            or bestfitcell[2, 0] != 0
            or bestfitcell[2, 1] != 0
            or nat_cell != nat_input
        ):
            ...
            if (nat_cell != nat_input) and (
                bestfitcell[0, 1] == 0
                and bestfitcell[0, 2] == 0
                and bestfitcell[1, 0] == 0
                and bestfitcell[2, 0] == 0
                and bestfitcell[2, 1] == 0
                and bestfitcell[1, 2] == 0
            ):
                # in this case we entered the bestfitcell in the input
                Acell_super, coords_super, t1, supercell_index = self.generate_cell(
                    newcell
                )
            else:
            ...
            natold = pos.shape[0]
            pos, A, types, newcell, forces = self.find_corresponding_non_diagonal(
                coords_super, pos, Acell_super, A, types, forces
            )
        ...
        if (
            bestfitcell[0] != self.supercell[0]
            or bestfitcell[1] != self.supercell[1]
            or bestfitcell[2] != self.supercell[2]
        ):  # put everything in correctly sized cell.  We double, triple, etc cells as long as the result is <= supercell
            A, types, pos, forces, stress, energy, factor = self.unfold_to_supercell(
                A, pos, types, forces, stress, energy
            )

            supercell_ref = [
                factor[0] * bestfitcell[0],
                factor[1] * bestfitcell[1],
                factor[2] * bestfitcell[2],
            ]
    ...
    def add_to_file_list(self, fl):
        ...
        for line in fl:
            ...
            A_big, types_big, pos_big, forces_big, stress_big, energy_big = (
                load_output_both(SP[0], self.relax_load_freq)
            )
            ...
            for cc, (A, types, pos, forces, stress, energy) in enumerate(
                zip(A_big, types_big, pos_big, forces_big, stress_big, energy_big)
            ):
                ...
                if len(SP) == 1:
                    ...
                    energy = energy - (
                        energy1 - self.energy_ref * pos.shape[0] / self.nat
                    )
                    ...
                    #          self.weights.append(float(SP[1]))
                    weight = float(SP[1])
                    bestfitcell_input = list(map(int, SP[2:5]))
                ...
                if self.verbosity == "High":
                    print(
                        "energy loaded without es adjustment: "
                        + str(energy)
                        + " "
                        + str(energy - self.energy_ref * np.prod(self.supercell))
                    )
                    print("forces direct")
                    print(forces)
                    print("stress")
                ...
                A, strain, rotmat, forces, stress = self.get_rot_strain(
                    A, refA, forces, stress
                )
                ...
                if vacancies != []:
                    ...
                    pos, types, correspond = self.fix_vacancies(
                        vacancies, pos, correspond, types
                    )
                ...
                if self.useewald == True:
                    energy_es, forces_es, stress_es = self.run_dipole_harm(
                        A, pos, refA, refpos
                    )
                    energy -= energy_es
                    ...
                    if self.verbosity == "High":
                        ...
                        print(
                            "energy short range "
                            + str(energy - self.energy_ref * np.prod(supercell_ref))
                        )
                ...
                for c in correspond:
                    u[c[1], :] = np.dot(pos[c[0], :] + c[2], A) - np.dot(
                        refpos[c[1], :], A
                    )
                ...
                energy_new = (
                    energy_orig
                    - self.energy_ref * np.prod(supercell_ref)
                    - self.doping_energy * types_count
                )

                if energy_new > self.energy_limit:
                    print(
                        "entry above energy limit : ",
                        energy_new,
                        " > ",
                        self.energy_limit,
                    )
                ...
                if vacancies != []:
                    ss = (
                        supercell_ref + supercell_ref
                    )  # vacancy case cannot shrink inputs to speed fitting, as the vacancies are given different random positions
                else:
    ...
    def fix_vacancies(self, vacancies, pos, correspond, types):
        ...
        for v in vacancies:
            ...
            pos_new[n, :] = (
                self.coords_super[v, :] + np.random.rand(3) * self.vacancy_param
            )
            n += 1
            if types != []:
                types.append("X")
    ...
    def find_corresponding(self, pos1, pos2, Aref=[]):
        ...
        (
            ...
            pos1, Aref, self.nat, natsuper, self.supercell, self.supercell_index, pos2
        )
        TIME.append(time.time())
        ...
        if nvac > 0:
            ...
            if self.vacancy == 0:
                print(
                    "We detected a vacancy when there isn't supposed to be one!!!!!!!!!! Try turning on self.vacancy or fixing problem"
                )
    ...
    def print_symmetry_info(self):
        ...
        for i, (rot, trans) in enumerate(
            zip(self.dataset["rotations"], self.dataset["translations"])
        ):
            nsymm += 1
            print("  --------------- %4d ---------------" % (i + 1))
            print("  rotation:")
    ...
    def atom_index(self, a, dim):
        ...
        if dim == 1:
            ...
            return [
                a / self.natsuper / self.natsuper,
                (a / self.natsuper) % self.natsuper,
                a % self.natsuper,
            ]
        elif dim == 4:
            return [
                a / self.natsuper / self.natsuper / self.natsuper,
                (a / self.natsuper / self.natsuper) % self.natsuper,
                (a / self.natsuper) % self.natsuper,
                a % self.natsuper,
            ]
            ...
            return [
                a / self.natsuper / self.natsuper / self.natsuper / self.natsuper,
                (a / self.natsuper / self.natsuper / self.natsuper) % self.natsuper,
                (a / self.natsuper / self.natsuper) % self.natsuper,
                (a / self.natsuper) % self.natsuper,
                a % self.natsuper,
    ...
    def atom_index_prim(self, a, dim):
        ...
        if dim == 1:
            ...
            return [
                a / self.nat / self.nat / self.nat / self.nat,
                (a / self.nat / self.nat / self.nat) % self.nat,
                (a / self.nat / self.nat) % self.nat,
                (a / self.nat) % self.nat,
                a % self.nat,
            ...
            return [
                a / self.nat / self.nat / self.nat / self.nat / self.nat,
                (a / self.nat / self.nat / self.nat / self.nat) % self.nat,
                (a / self.nat / self.nat / self.nat) % self.nat,
                (a / self.nat / self.nat) % self.nat,
                (a / self.nat) % self.nat,
            ...
            return [
                a / self.nat / self.nat / self.nat / self.nat / self.nat / self.nat,
                (a / self.nat / self.nat / self.nat / self.nat / self.nat) % self.nat,
                (a / self.nat / self.nat / self.nat / self.nat) % self.nat,
                (a / self.nat / self.nat / self.nat) % self.nat,
                (a / self.nat / self.nat) % self.nat,
    ...
    def ss_index_dim(self, a, dim):
        ...
        if dim == 1:
            ...
            return [
                a / ncells / ncells / ncells / ncells / ncells,
                (a / ncells / ncells / ncells / ncells) % ncells,
                (a / ncells / ncells / ncells) % ncells,
                (a / ncells / ncells) % ncells,
                (a / ncells) % ncells,
            ...
            return [
                a / ncells / ncells / ncells / ncells / ncells / ncells,
                (a / ncells / ncells / ncells / ncells / ncells) % ncells,
                (a / ncells / ncells / ncells / ncells) % ncells,
                (a / ncells / ncells / ncells) % ncells,
                (a / ncells / ncells) % ncells,
            ...
            return [
                a / ncells / ncells / ncells / ncells / ncells / ncells / ncells,
                (a / ncells / ncells / ncells / ncells / ncells / ncells) % ncells,
                (a / ncells / ncells / ncells / ncells / ncells) % ncells,
                (a / ncells / ncells / ncells / ncells) % ncells,
                (a / ncells / ncells / ncells) % ncells,
    ...
    def index_ijk(self, ijk, dim):
        ...
        if dim == 1:
            ...
            return (
                ijk[0] * 27 * 3 + ijk[1] * 9 * 3 + ijk[2] * 3 * 3 + ijk[3] * 3 + ijk[4]
            )
        elif dim == 6:
            return (
                ijk[0] * 27 * 3 * 3
    ...
    def prepare_dim(self, dim):
        ...
        if dim > 0:
            ...
            for x in permutations(list(range(dim))):
                P.append(x)
                if self.verbosity == "High":
                    print(x)
            print()
    ...
    def apply_sym_phi(
        self, dim, dist_cutoff, bodycount=100, dist_cutoff_twobody=0.001, limit_xy=False
    ):
        ...
        if (
            dimtot <= 4
        ):  # low dimensions, brute force case. Handles 2body-only term requirements
            atomlist_long = np.zeros((natdim, dimtot + 1), dtype=int, order="F")
            find_nonzero_fortran(
                atomlist_long,
                nonzero_atoms_arr,
                self.dist_array,
                bodycount,
                float(dist_cutoff),
                float(dist_cutoff_twobody),
                self.natsuper,
                natdim,
                dimtot,
            )
            ...
            nonzero_atoms, atomlist = find_nonzero_highdim(
                self, dimtot, dist_cutoff, bodycount, float(dist_cutoff_twobody)
            )
        ...
        for aa in range(nonzero_atoms):  # loop over sets of atoms
            ...
            if dimtot > 2 and insidecutoff == True and body > 2:  # pbc issues
                ...
                for c, at1 in enumerate(atoms):
                    delta[c, :] = (
                        -self.dist_array_R[atoms[0], at1, :] + self.coords_super[at1, :]
                    )
                ...
                for c1 in range(dimtot):
                    for c2 in range(dimtot):
                        ...
                        if (
                            d > dist_cutoff
                        ):  # and not((dim[0] == 2 and (dim[1] == 1 or dim[1] == 2)) or (dim[0] == 1 and dim[1] == 2)):###and dim != [1,2] and dim != [2,2] and dim != [2,1]:
                            if self.verbosity == "High":
                ...
                for ps in range(permutedim_s):
                    for pk in range(permutedim_k):
                        for count in range(len(self.CORR)):
                            atoms_new = ATOMS_P_alt[
                                (
                                    (aa) * permutedim_k * permutedim_s
                                    + ps * permutedim_k
                                    + pk
                                )
                                * self.nsymm
        ...
        if not approx_anharm:
            print(
                "Independent groups of atoms (atom numbers, distance, nbody): "
                + str(dim)
            )
        else:
        ...
        if not approx_anharm:
            nind, ntotal_ind, ngroups, Tinv, nonzero_list = analyze_syms(
                np.array(self.dataset["rotations"], dtype=int),
                groups,
                np.array([permutedim_s, permutedim_k], dtype=int),
                [Parray_s, Parray_k],
                tensordim_k,
                natdim,
                np.array(dim, dtype=int),
                ATOMS_P_alt,
                self,
                atomlist,
                limit_xy=limit_xy,
    ...
    def pre_setup(self):
        ...
        (
            ...
            self.UTT0,
            self.UTT_ss,
            self.atomcode_different_supercell,
        ) = pre_setup_cython(self)
    ...
    def setup_lsq_fast(
        self, nind, ntotal_ind, ngroups, Tinv, dim, dist_cutoff, nonzero_list
    ):
        ...
        if dim[1] >= 0:
            permutedim_k, natdim_k, natdim_1_k, tensordim_k, P_k = self.prepare_dim(
                dim[1]
            )
        else:
            tensordim_k = 1
    ...
    def do_lsq(self, UMAT, ASR):
        ...
        for c, [forces, stress, energy, A, w, type_var] in enumerate(
            zip(self.F, self.stress, self.energy, self.Alist, self.weights, self.TYPES)
        ):
            ...
            for at in range(forces.shape[0]):
                for i in range(3):
                    ...
                    if True:  # we try to eliminate duplicate fitting data
                        ...
                        if len(nind_indpt) > 0:
                            temp = np.abs(
                                forces[at, i] - forces_ind[0 : len(nind_indpt)]
                            )
                            found = False
                                ...
                                for n in range(len(nind_indpt)):
                                    if temp[n] < 1e-7:
                                        ...
                                        if (
                                            ...
                                            < 1e-7
                                        ):  # U matricies also match, duplicate
                                            # found a copy
                                            found = True
                                            break
            ...
            if self.usestress:  # now add stress
                ...
                for i in range(3):
                    for j in range(i, 3):
                        Fmat[c * self.unitsize + self.natsupermax * 3 + t] = (
                            stress[i, j] * self.alat**-1 * self.energy_weight
                        )  # * np.linalg.det(A)
                        UMAT[c * self.unitsize + self.natsupermax * 3 + t, :] = (
                            UMAT[c * self.unitsize + self.natsupermax * 3 + t, :]
                            / abs(np.linalg.det(A))
                            * self.energy_weight
                        )
            ...
            if self.useenergy:  # now add energy
                ...
                if c in self.exact_constraint:
                    exact_constraints.append(
                        [copy.copy(UMAT[ind, :]), energy * self.energy_weight]
                    )
        ...
        if (self.vacancy == 2 or self.vacancy == 1) and self.useasr == True:
            ...
            if len(vacancy_constraints) > 0:
                ...
                ASR_vac = np.zeros(
                    (len(vacancy_constraints), ASR.shape[1]), dtype=float
                )
        ...
        if self.useasr:
            ...
            if len(exact_constraints) > 0:
                exact_mat = np.zeros(
                    (len(exact_constraints), ASR.shape[1]), dtype=float
                )
                ...
                constraint_values = np.concatenate(
                    (constraint_values, exact_col), axis=0
                )
        ...
        if self.useenergy and len(self.energy_differences) > 0:
            ...
            UMAT_add = np.zeros(
                (len(self.energy_differences), UMAT.shape[1]), dtype=float
            )
            ...
            for n1, n2 in self.energy_differences:
                ...
                UMAT_add[c_ediff, :] = (
                    (UMAT[ind1, :] - UMAT[ind2, :])
                    / self.energy_weight
                    * self.energy_differences_weight
                )
                Fmat_add[c_ediff] = (
                    (Fmat[ind1] - Fmat[ind2])
                    / self.energy_weight
        ...
        print()
        print(
            "UMAT out of total size "
            + str(Fmat.shape[0])
            + ", we keep "
            + str(len(keep))
            + ", rest are duplicate entries"
        )
        ...
        if asr == True:
            ...
            if nnonzero == 0:
                ...
                if (
                    self.regression_method.lower() == "lasso"
                ):  # lasso requires lasso from sklearn
                    print("using lasso")
                    alpha = self.alpha
                    #          alpha = 0.1e-11
                    ...
                    A3 = np.zeros(
                        (UMAT.shape[0] + Aprime.shape[0], UMAT.shape[1]), dtype=float
                    )
                    ...
                    A3[UMAT.shape[0] :, :] = (
                        Aprime / 100.0
                    )  # this line adds in the constraint matrix to the least squares fitting, so the lasso will approximatly obey the constraints
                    ...
                    if (
                        self.post_process_constraint
                    ):  # in this case, we try to fix the constraints by adding to coefficents as
                        ...
                        Az = np.zeros(
                            (Aprime.shape[0] + len(zeros), Aprime.shape[1]), dtype=float
                        )
                        ...
                        print(
                            str(np.sum(abs(phi_indpt) > 1e-9))
                            + " nonzero components from lasso out of "
                            + str(phi_indpt.shape[0])
                        )
                        ...
                        phi_indpt_postlasso = self.lsq.fit_old(
                            -U_postlasso, Fmat, ASR_postlasso, constraint_values
                        )
                    ...
                    self.regression_method.lower() == "rfe"
                    or self.regression_method.lower() == "rfecv"
                    or self.regression_method.lower() == "recursive_feature_elimination"
                ):
                    ...
                    phi_indpt, support = self.run_rfe(
                        -UMAT, Fmat, Aprime, constraint_values
                    )
        ...
        if self.verbosity == "High":
            ...
            for x in range(N):
                print(
                    str(predicted_forces_energy[x])
                    + "\t"
                    + str(Fmat[x])
                    + "\t"
                    + str(predicted_forces_energy[x] - Fmat[x])
                )
            print("---")

        print(
            "sum_error rms : "
            + str(np.sum((predicted_forces_energy - Fmat) ** 2) ** 0.5)
        )
        print()
        print()
    ...
    def run_rfe(self, X, y, ASR, vals):
        ...
        if self.num_keep <= 0:
            ...
            n_features_to_select_max = (
                n_features - (nscore - (np.argmax(scores_mean) + 1)) * step
            )
            ...
            n_features_to_select_se_median = (
                n_features - (nscore - (i_se_median + 1)) * step
            )
            ...
            if self.rfe_type == "max-mean":
                ...
                print(
                    "Using RFE, we select ",
                    nselect,
                    " features, which give us the max CV score (mean of folds)",
                )
                ...
                print(
                    "Using RFE, we select ",
                    nselect,
                    ' features, which give us the "good enough" CV score (mean of folds - small tolerance)',
                )
                ...
                print(
                    "Using RFE, we select ",
                    nselect,
                    ' features, which give us the "good enough" CV score (median of folds - small tolerance)',
                )
            else:
                ...
                print(
                    "Using RFE, we select ",
                    nselect,
                    " features, which give us the best CV score (median of folds)",
                )

            print()
            print(
                "Using recursive feature extraction, we keep ",
                nselect,
                " features out of ",
                n_features,
                " which means ",
        ...
        score_final, coefs_final, support_tf = self.lsq.single_rfe(
            X, y, np.arange(X.shape[0]), np.arange(X.shape[0]), nselect, step=step
        )
        ...
        if self.verbosity.lower() != "minimal" and self.num_keep <= 0:
            ...
            (h4,) = plt.plot(
                x[i_max], scores_mean[i_max], "r.", markersize=12, label="max-mean"
            )
            (h5,) = plt.plot(
                x[i_se], scores_mean[i_se], "g.", markersize=10, label="good-mean"
            )
            (h6,) = plt.plot(
                x[i_median],
                scores_median[i_median],
                "b.",
                markersize=9,
                label="max-median",
            )
    ...
    def supercell_index_f(self, ss):
        return (
            ss[0] * self.supercell[1] * self.supercell[2]
            + ss[1] * self.supercell[2]
            + ss[2]
        )

    def index_supercell_f(self, ssind):
        return [
            ssind / (self.supercell[1] * self.supercell[2]),
            (ssind / self.supercell[2]) % self.supercell[1],
            ssind % (self.supercell[2]),
        ]

    def calculate_energy_force_stress(
        self, A, coords, types, dims, phis, phi_tensors, nonzero, cell=[]
    ):
        ...
        energy_sr, forces_sr, stress_sr = calculate_energy_fortran(
            self, A, coords, types, dims, [], phi_tensors, nonzero, supercell_input=cell
        )
    ...
    def run_montecarlo_test(
        ...
        starting_energy = run_montecarlo(
            self,
            0.0,
            use_all,
            beta,
    ...
    def run_montecarlo(
        ...
        print(
            "BASIC MC ANALYSIS. Feel free to program what you want instead, I cannot predict it. Quantites are currently in atomic units (the Rydberg type) or fractional "
        )
            ...
            for at in range(struct_all.shape[0]):
                if at in self.cluster_sites:
                    if self.magnetic == 2:
                        print(
                            self.reverse_types_dict[
                                int(round(math.cos(cluster_all[at, cell, 0, -1])))
                            ]
                            + "\t"
                            + str(struct_all[at, cell, 0, -1])
                            + "   "
                            + str(struct_all[at, cell, 1, -1])
                            + "  "
                            + str(struct_all[at, cell, 2, -1])
                            + "         "
                            + str(
                                [
                                    math.cos(cluster_all[at, cell, 1, -1])
                                    * math.sin(cluster_all[at, cell, 0, -1]),
                                    math.sin(cluster_all[at, cell, 1, -1])
                                    * math.sin(cluster_all[at, cell, 0, -1]),
                                    math.cos(cluster_all[at, cell, 0, -1]),
                                ]
                            )
                        )
                    else:
                        print(
                            self.reverse_types_dict[
                                int(round(cluster_all[at, cell, -1]))
                            ]
                            + "\t"
                            + str(struct_all[at, cell, 0, -1])
                            + "   "
        ...
        for i in range(3):
            Asuper[i, :] = np.dot(
                self.Acell[i, :] * supercell[i], np.eye(3) + strain_all[:, :, -1]
            )
        ...
        print(
            "AVERAGE STRUCTURE (entire supercell, avg over steps, crystal coordinates)"
        )
        print("-------------")
        for cell in range(struct_all.shape[1]):
            for at in range(struct_all.shape[0]):
                if at in self.cluster_sites:
                    if self.magnetic == 2:
                        print(
                            self.reverse_types_dict[
                                int(round(math.cos(avg_cluster_all[at, cell, 0])))
                            ]
                            + "\t"
                            + str(avg_struct_all[at, cell, 0])
                            + "   "
                            + str(avg_struct_all[at, cell, 1])
                            + "  "
                            + str(avg_struct_all[at, cell, 2])
                            + "         "
                            + str(
                                [
                                    math.cos(avg_cluster_all[at, cell, 1])
                                    * math.sin(avg_cluster_all[at, cell, 0]),
                                    math.sin(avg_cluster_all[at, cell, 1])
                                    * math.sin(avg_cluster_all[at, cell, 0]),
                                    math.cos(avg_cluster_all[at, cell, 0]),
                                ]
                            )
                        )
                    else:
                        if (
                            int(round(avg_cluster_all[at, cell]))
                            in self.reverse_types_dict
                        ):
                            t = self.reverse_types_dict[
                                int(round(avg_cluster_all[at, cell]))
                            ]
                        else:
                            t = "X"
        ...
        if use_all[0]:
            ...
            print(
                "STANDARD DEV (entire supercell, std over steps, crystal coordinates)"
            )
            print("-------------")
            for cell in range(struct_all.shape[1]):
                for at in range(struct_all.shape[0]):
                    if at in self.cluster_sites:
                        if self.magnetic == 2:
                            print(
                                ...
                                + "\t"
                                + str(avg_std_all[at, cell, 0])
                                + "   "
                                + str(avg_std_all[at, cell, 1])
                                + "  "
                                + str(avg_std_all[at, cell, 2])
                                + "         "
                                + str(
                                    [
                                        math.cos(avg_cluster_all[at, cell, 1])
        ...
        if use_all[0]:
            ...
            for cell222 in range(8):
                ...
                for at in range(struct_all.shape[0]):
                    for i in range(3):
                        struct_222[cell222, at, i] = np.mean(
                            avg_struct_all[at, cells, i] - coords_ref[at, cells, i]
                        ) * (supercell[i] / 2.0)
        ...
        if use_all[1]:
            ...
            print(
                "Strain sorted: "
                + str(np.mean(st_sorted, 1))
                + " abs: "
                + str(np.mean(st_sorted_offcenter, 1))
            )
            print()
            print("------------------------------------------------------------")
        ...
        if use_all[1]:
            ...
            print()
            print(
                "All quantities are the fixed cell (fixed to starting strain) versions"
            )
            print()
        ...
        if use_all[1]:
            ...
            for st in range(strain_all.shape[2]):  # convert to voight notation
                ...
                for i in range(3):
                    for j in range(3):
                        ...
                        if vij != -1:
                            if vij <= 2:
                                strain_steps[st, vij] = strain_all[
                                    i, j, st
                                ]  # - strain_average[i,j]
                            elif vij > 2:
                                strain_steps[st, vij] = (
                                    strain_all[i, j, st] * 2.0
                                )  # - strain_average[i,j])*2.0
            ...
            for i in range(6):
                for j in range(6):
                    ...
                    compliance[i, j] = (
                        ...
                        * beta
                        * np.mean(
                            (strain_steps[:, i] - strain_avg_voight[i])
                            * (strain_steps[:, j] - strain_avg_voight[j])
                        )
                    )

            if use_all[2]:
                print(
                    "WARNING: ELASTIC PROPERTIES FOR GRAND CANONICAL MC ARE PROBABLY NOT CORRECT"
                )
            ...
            if abs(np.linalg.det(compliance)) > 1e-12:
                ...
                try:
                    ...
                    print()
                    print(
                        "Bulk Modulus Voight Average (K_V) = "
                        + str(
                            (
                                elastic[0, 0]
                                + elastic[1, 1]
                                + elastic[2, 2]
                                + 2 * (elastic[0, 1] + elastic[1, 2] + elastic[0, 2])
                            )
                            / 9.0
                        )
                    )
                    print(
                        "Shear modulus Voight Average (G_V) = "
                        + str(
                            (
                                elastic[0, 0]
                                + elastic[1, 1]
                                + elastic[2, 2]
                                - (elastic[0, 1] + elastic[1, 2] + elastic[0, 2])
                                + 3 * (elastic[3, 3] + elastic[4, 4] + elastic[5, 5])
                            )
                            / 15.0
                        )
                    )
                    ...
                    print(
                        "compliance tensor not invertible, det = "
                        + str(np.linalg.det(compliance))
                    )
        ...
        if self.useewald == True:
            if use_all[0]:
                polarization_reduced_avg, polarization_physical_avg = (
                    self.calc_polarization(struct_average + self.coords_hs, A_avg)
                )
                ...
                if use_all[2]:
                    print(
                        "WARNING: DIELECTRIC PROPERTIES FOR GRAND CANONICAL MC ARE PROBABLY NOT CORRECT"
                    )
                ...
                for st in range(struct_all.shape[3]):
                    struct_average_step = np.mean(
                        struct_all[:, :, :, st] - coords_ref, 1
                    )
                    for i in range(3):
                        struct_average_step[:, i] = (
                            struct_average_step[:, i] * supercell[i]
                        )
                ...
                print(
                    "Change in polarization magnitude: "
                    + str(np.mean(polarization_reduced_mag))
                )
                print()
                print("Chi ionic")
                ...
                const = (
                    1.0 / epsilon_naught * 2.0 * vol0 * beta * np.prod(supercell)
                )  #  e^2=2

                for i in range(3):
                    for j in range(3):
                        diel_ionic[i, j] = const * np.mean(
                            (polarization_reduced[:, i] - polarization_reduced_avg[i])
                            * (polarization_reduced[:, j] - polarization_reduced_avg[j])
                        )
                #        print diel_ionic

                diel_ionic_magnitude = const * np.mean(
                    polarization_reduced_mag**2 - np.mean(polarization_reduced_mag) ** 2
                )
            ...
            if use_all[0] and use_all[1]:  # piezoelectric
                ...
                strain_isotropic_mean = (
                    abs(np.linalg.det(strain_average)) ** 0.33333333333333
                )
                for st in range(strain_all.shape[2]):  # convert to voight notation

                    strain_isotropic[st] = (
                        abs(np.linalg.det(strain_all[:, :, st])) ** 0.33333333333
                        - strain_isotropic_mean
                    )
                    ...
                    for i in range(3):
                        for j in range(3):
                            ...
                            if vij != -1:
                                if vij <= 2:
                                    strain_steps[st, vij] = (
                                        strain_all[i, j, st] - strain_average[i, j]
                                    )
                                elif vij > 2:
                                    strain_steps[st, vij] = (
                                        strain_all[i, j, st] - strain_average[i, j]
                ...
                vol = abs(
                    np.linalg.det(np.dot(self.Acell, np.eye(3) + strain_all[:, :, st]))
                )
                    ...
                    for j in range(6):
                        piezo[i, j] = (
                            ...
                            * np.mean(
                                strain_steps[:, j]
                                * (
                                    polarization_reduced[:, i]
                                    - polarization_reduced_avg[i]
                                )
                            )
                        )
                ...
                print(
                    ...
                    + str(
                        ...
                        * np.mean(
                            strain_isotropic[:]
                            * (
                                polarization_reduced_mag[:]
                                - np.mean(polarization_reduced_mag)
                            )
                        )
                    )
                )
        ...
        if use_all[2]:
            ...
            if self.magnetic <= 1:
                print(
                    "Mean Cluster (cluster sites only)",
                    np.mean(cluster_all[self.cluster_sites, :, :]),
                )
                ...
                print(
                    "Mean over steps of |Cluster| (cluster sites only) "
                    + str(np.mean(T))
                )
                ...
                for step in range(cluster_all.shape[2]):
                    ...
                    mean_cluster[step] = mean_cluster_current / float(
                        ncells * len(self.cluster_sites)
                    )
                    mean_abs_cluster[step] = abs(mean_cluster_current) / float(
                        ncells * len(self.cluster_sites)
                    )
                    ...
                    mean_111_cluster[step] = mean_111_cluster_current / float(
                        ncells * len(self.cluster_sites)
                    )
                    mean_abs_111_cluster[step] = abs(mean_111_cluster_current) / float(
                        ncells * len(self.cluster_sites)
                    )
                ...
                print(
                    "Mean_abs_staggered_cluster: " + str(np.mean(mean_abs_111_cluster))
                )
                print()
                print()
                print("Mean_cluster squared: " + str(np.mean(mean_cluster**2)))
                print(
                    "Mean_staggered_cluster squared: "
                    + str(np.mean(mean_111_cluster**2))
                )
                ...
                print(
                    "Chi: "
                    + str(
                        (np.mean(mean_cluster**2) - np.mean(mean_abs_cluster) ** 2)
                        * beta
                        * bohr_magneton
                        * np.prod(supercell)
                    )
                ...
                print(
                    ...
                    + str(
                        (
                            np.mean(mean_111_cluster**2)
                            - np.mean(mean_abs_111_cluster) ** 2
                        )
                        * beta
                        * bohr_magneton
                        * np.prod(supercell)
                ...
                print(
                    "Chi uses bohr_magneton = "
                    + str(bohr_magneton)
                    + " and unit spin magnitude"
                )
                print()
                print(
                    "Cumulant: "
                    + str(
                        1
                        - np.mean(mean_cluster**4 + 1e-9)
                        / 3.0
                        / np.mean(mean_cluster**2 + 1e-9) ** 2
                    )
                )
                print(
                    "Cumulant_staggered: "
                    + str(
                        1
                        - np.mean(mean_111_cluster**4 + 1e-9)
                        / 3.0
                        / np.mean(mean_111_cluster**2 + 1e-9) ** 2
                    )
                )

            elif self.magnetic == 2:
                print(
                    "Mean Cluster (cluster sites only) (theta, phi)",
                    np.mean(cluster_all[self.cluster_sites, :, 0, :]),
                    np.mean(cluster_all[self.cluster_sites, :, 1, :]),
                )
                if cluster_all.shape[2] > 1:
                    std_cluster = [
                        np.std(cluster_all[self.cluster_sites, :, 0, :], axis=2),
                        np.std(cluster_all[self.cluster_sites, :, 1, :], axis=2),
                    ]
                ...
                for step in range(cluster_all.shape[3]):
                    ...
                    for s in range(ncells):
                        ...
                        stag = (-1) ** (ss[0] + ss[1] + ss[2])
                        for at in self.cluster_sites:
                            mean_cluster_current += math.cos(
                                cluster_all[at, s, 0, step]
                            )
                            mean_111_cluster_current += stag * math.cos(
                                cluster_all[at, s, 0, step]
                            )
                ...
                print(
                    "Mean_staggered_cluster z comp: " + str(np.mean(mean_111_cluster))
                )
                print(
                    "Mean_abs_staggered_cluster z comp: "
                    + str(np.mean(mean_abs_111_cluster))
                )
                print()
                ...
                for step in range(cluster_all.shape[3]):
                    ...
                    for s in range(ncells):
                        ...
                        stag = (-1) ** (ss[0] + ss[1] + ss[2])
                        for at in self.cluster_sites:
                            x = math.sin(cluster_all[at, s, 0, step]) * math.cos(
                                cluster_all[at, s, 1, step]
                            )
                            y = math.sin(cluster_all[at, s, 0, step]) * math.sin(
                                cluster_all[at, s, 1, step]
                            )
                            ...
                            mean_111_cluster_current2 += [
                                stag * x**2,
                                stag * y**2,
                                stag * z**2,
                            ]

                    mean_cluster[step] = np.sum(
                        mean_cluster_current**2
                    ) ** 0.5 / float(ncells * len(self.cluster_sites))
                    #          mean_abs_cluster[step] = abs(mean_cluster_current)/float(ncells*len(self.cluster_sites))

                    mean_111_cluster[step] = np.sum(
                        mean_111_cluster_current**2
                    ) ** 0.5 / float(ncells * len(self.cluster_sites))
                ...
                print(
                    "Mean_staggered_cluster squared: " + str(np.mean(mean_111_cluster2))
                )
                print()
                bohr_magneton = 2.0**0.5
                print()
                ...
                print(
                    "Chi: "
                    + str(
                        (np.mean(mean_cluster2) - np.mean(mean_cluster) ** 2)
                        * beta
                        * bohr_magneton
                        * np.prod(supercell)
                    )
                ...
                print(
                    "Chi staggered: "
                    + str(
                        (np.mean(mean_111_cluster2) - np.mean(mean_111_cluster) ** 2)
                        * beta
                        * bohr_magneton
                        * np.prod(supercell)
                    )
                ...
                print(
                    ...
                    + str(
                        1
                        - np.mean(mean_cluster2**2) / 3.0 / np.mean(mean_cluster2) ** 2
                    )
                )
                print(
                    "Cumulant_staggered: "
        ...
        print(
            "Heat Capacity: "
            + str(beta**2 * (np.mean(energies**2) - np.mean(energies) ** 2))
        )
        ...
        if below_2 > 0:
            ...
            print(
                "So there are approx. "
                + str(round(indept))
                + " indept. samples (in energy)"
            )
            t = np.std(energies) / math.sqrt(float(nen) * 2 / float(below_2))
            print(
                "The energy error bars are ~ +/- "
                + str(np.std(energies) / math.sqrt(float(nen) * 2 / float(below_2)))
            )
            print("or " + str(t / float(ncells * self.nat)) + " per atom")
            print()
            if indept < 5.1:
                print(
                    "Warning, there might be a very small number of independant MC samples."
                )
            ...
            print(
                "Warning, either the energies are very strongly correlated throughout the Monte Carlo sampling. "
            )
        ...
        if use_all[0]:
            ...
            for step in range(struct_all.shape[3]):
                ...
                for cell in range(ncells):
                    struct_cart[:, cell, :, step] = np.dot(
                        struct_all[:, cell, :, step] - coords_ref[:, cell, :], Acell
                    )
            ...
            for at in range(self.nat):
                for i in range(3):
                    z[:] = 0.0
                    for cell in range(ncells):
                        z[cell * nsamples : (cell + 1) * nsamples] = struct_cart[
                            at, cell, i, :
                        ]
                    z = z - np.mean(z)
                    ...
                    for bi in range(counts.shape[0]):
                        print(
                            "{:10.7f} {:10.7f} {:10.7f}".format(
                                counts[bi], bins[bi], bins[bi + 1]
                            )
                        )
                ...
                z[:] = 0.0
                for cell in range(ncells):
                    z[cell * nsamples : (cell + 1) * nsamples] = (
                        struct_cart[at, cell, 0, :] + struct_cart[at, cell, 1, :]
                    ) / 2.0**0.5
                z = z - np.mean(z)
                ...
                for cell in range(ncells):
                    z[cell * nsamples : (cell + 1) * nsamples] = (
                        struct_cart[at, cell, 0, :]
                        + struct_cart[at, cell, 1, :]
                        + struct_cart[at, cell, 2, :]
                    ) / 3.0**0.5
                z = z - np.mean(z)
        ...
        for l in self.firstnn_list:
            ...
            for cell in range(ncells):
                ...
                for step in range(struct_all.shape[3]):
                    ...
                    d[:] = np.dot(
                        (
                            struct_all[at1, cell, :, step]
                            - struct_all[at2, cell_delta, :, step]
                            + shift
                        ),
                        Acell,
                    )
    ...
    def load_harmonic_new(
        ...
        self.dyn.load_harmonic(
            ...
            zeff=zeff,
            A=self.Acell,
            coords=self.coords_hs,
            types=self.coords_type,
        )
        ...
        correspond, vacancies = self.find_corresponding(
            self.dyn.pos_crys, self.coords_hs
        )
    ...
    def get_dipole_harm(self, refA, refpos, low_memory=False):
        ...
        H_R, harm_normal, H_Rp, harm_normalp, H_Rpp, harm_normalpp, H_Q = (
            self.dyn.supercell_fourier_make_force_constants_derivative_harm(
                refA, refpos, low_memory
            )
        )
                    ...
                    for jj in range(3):
                        for a in range(self.nat):
                            for b in range(self.nat):
                                v[i, j, ii, jj] += (
                                    harm_normalpp[a * 3 + i, b * 3 + j, ii, jj].real
                                    / 2.0
                                )  # forcing constraint to be obeyed
                                v[ii, jj, i, j] += (
                                    harm_normalpp[a * 3 + i, b * 3 + j, ii, jj].real
                                    / 2.0
                                )
        ...
        for a in range(3):
            for g in range(3):
                for b in range(3):
                    for l in range(3):
                        elastic_constants[a, g, b, l] = (
                            v[a, b, g, l] + v[b, g, a, l] - v[b, l, a, g]
                        )
    ...
    def run_dipole_harm(self, A, pos, refA=[], refpos=[]):
        ...
        energy, forces, stress = dipole_dipole(
            u, strain, v, vf, harm_normal.real, volsuper, ncells, natsuper, self.nat
        )
    ...
    def write_harmonic_qe(
        self, filename, phi, nonzero, cell_towrite=[], dontwrite=False
    ):
        ...
        for nz in range(nonzero.shape[0]):
            ...
            phi_matrix[
                atoms[0], atoms[1], cell[0], cell[1], cell[2], ijk[0], ijk[1]
            ] += phi[nz]
        ...
        amu_to_au = (
            self.amu / self.me / 2.0
        )  # m_amu / m_e  #factor of 2 for ryd units apparently
        ...
        for i, [name, coords] in enumerate(
            zip(self.coords_type, np.dot(self.coords_hs, self.Acell) / alen)
        ):
            ...
            for ijk2 in range(3):
                for a1 in range(self.nat):
                    for a2 in range(self.nat):
                        ...
                        retst += self.output_integers(
                            [ijk1 + 1, ijk2 + 1, a1 + 1, a2 + 1]
                        )  # identifying stuff, ijk then atom
                        for c2 in range(cell_towrite[2]):
                            for c1 in range(cell_towrite[1]):
                                for c0 in range(cell_towrite[0]):
                                    ...
                                    retst += (
                                        ...
                                        + "  "
                                        + str(
                                            phi_matrix[a2, a1, c0, c1, c2, ijk2, ijk1]
                                        )
                                        + "\n"
                                    )
    ...
    def write_thirdorder_shengbte(self, filename, phi, nonzero):
        ...
        phi_matrix = np.zeros(
            (
                ...
                self.nat,
                self.nat,
                self.supercell[0] * 2 + 1,
                self.supercell[1] * 2 + 1,
                                    ...
                                    for a2 in range(self.nat):
                                        for a3 in range(self.nat):
                                            ...
                                            if s > 1e-7 or (
                                                a1 == a2
                                                and a2 == a3
                                                and c0 == self.supercell[0]
                                                and c1 == self.supercell[1]
                                                and c2 == self.supercell[2]
                                                and c0a == self.supercell[0]
                                                and c1a == self.supercell[1]
                                                and c2a == self.supercell[2]
                                            ):
                                                #                      if True:
                                                nonzero += 1
                                                string_temp = str(nonzero) + "\n"
                                                A1 = np.dot(
                                                    [c0, c1, c2] - self.supercell,
                                                    self.Acell * self.ang,
                                                )
                                                A2 = np.dot(
                                                    [c0a, c1a, c2a] - self.supercell,
                                                    self.Acell * self.ang,
                                                )
                                                string_temp += (
                                                    str(A1[0])
    ...
    def find_best_fit_cell(self, A):
        ...
        for i in range(3):
            for j in range(3):
                n1 = int(
                    np.ceil(
                        np.dot(A[i, :], self.Acell[j, :])
                        / np.linalg.norm(self.Acell[j, :]) ** 2
                    )
                )
                if n1 > n:
                    n = n1
        ...
        dmin = [1000000000.0, 1000000000.0, 1000000000.0]
    ...
    def find_corresponding_non_diagonal(
        self, pos_ref, pos_new, A_ref, A_new, types, forces=[]
    ):
        ...
        dist_A = np.ones(3, dtype=float) * 1000000000.0
        combo = [[], [], []]
        ...
        for x in range(-n, n + 1):
            for y in range(-n, n + 1):
                for z in range(-n, n + 1):
                    ...
                    pos_big_cart[nat * c : nat * (c + 1), :] = np.dot(
                        pos_new + r, A_new
                    )
                    c += 1
        ...
        if any(dist_A > 3.0):
            print(
                "WARNING - large lattice vector mismatch. Are you sure your non-orthogonal cell fits inside the supercell?"
            )
            print("Found:")
            print(combo)
            print(dist_A)
        ...
        for c in range(pos_ref.shape[0]):
            ...
            distmat = (
                np.sum(
                    (
                        np.tile(pos_ref_cart[c, :], (nat * (2 * n + 1) ** 3, 1))
                        - pos_big_cart
                    )
                    ** 2,
                    1,
    ...
    def elastic_constants(self, phi, nonzero):
        ...
        if self.useewald == True:
            phi_dip, Cij_es, interaction, hq = self.get_dipole_harm(
                self.Acell, self.coords_hs
            )
            interaction *= -1.0
        ...
        correspond, vacancies = self.find_corresponding(
            self.coords_super, self.coords_super
        )
        for [c0, c1, RR] in correspond:
            sss = self.supercell_index[c1]
            us0[c1 % self.nat, sss, :] = np.dot(
                self.coords_super[c1, :] - RR, self.Acell_super
            )
        ...
        for nz in range(nonzero.shape[0]):
            ...
            nsym = float(
                len(
                    self.moddict_prim[
                        na * self.nat * self.ncells**2
                        + sa * self.ncells * self.nat
                        + nb * self.ncells
                        + sb
                    ]
                )
            )
            for m_count, m in enumerate(
                self.moddict_prim[
                    na * self.nat * self.ncells**2
                    + sa * self.ncells * self.nat
                    + nb * self.ncells
                ...
                for i in range(3):
                    interaction[nb, ijk[0], ijk[1], i] += (
                        phi[nz] * dcart[i] / nsym
                    )  # /2.0
                    for ii in range(3):
                        bracket[ijk[0], ijk[1], i, ii] += (
                            -dcart[i] * dcart[ii] * phi[nz] / nsym
                        )
                        a = ijk[0]
                        b = ijk[1]
                        g = i
                ...
                for a2 in range(self.nat):
                    for j in range(3):
                        ...
                        if abs(phi_gamma[a1, i, a2, j]) > 1e-8:
                            phi_gamma_v2[a1 * 3 + i, a2 * 3 + j] = phi_gamma[
                                a1, i, a2, j
                            ]
                        else:
        ...
        Cij_relaxed = Cij_voight - np.dot(
            interaction_v2.T, np.dot(pseudoinv, interaction_v2)
        )
        ...
        if self.useewald == True:
            ...
            piezo_relaxed = (
                -1.0
                / abs(np.linalg.det(self.Acell))
                * np.dot(interaction_v2.T, np.dot(pseudoinv, zborn))
            )
            ...
            diel_relaxed = diel + 2.0 * 4 * np.pi / abs(
                np.linalg.det(self.Acell)
            ) * np.dot(zborn.T, np.dot(pseudoinv, zborn))

            diel_relaxed_stress = diel_relaxed + abs(
                np.linalg.det(self.Acell)
            ) * 2.0 * 4 * np.pi * np.dot(piezo_relaxed.T, np.dot(S, piezo_relaxed))
            ...
            print(
                "piezo e_ij (strain=0, only the relaxed ion contribution, need to add fixed ion contribution aka the electronic contribution)"
            )
            print("------------------")
    ...
    def reconstruct_fcs_nonzero_phi_relative(
        self, phi_ind_dim, ngroups, nind, trans, d, nonzero_list
    ):
        ...
        nz, phi_nz = reconstruct_fcs_nonzero_phi_relative_cython(
            self, phi_ind_dim, ngroups, nind, trans, d, nonzero_list
        )
...
class lsq_constraint:
    ...
    def fit(self, U, F, features=None):
        ...
        if self.ASR is not None:
            return self.fit_old(
                U[:, features], F, self.ASR[:, features], self.vals, False
            )
        else:
    ...
    def fit_old(self, U, F, A, constraint_values, eliminated=False):
        ...
        if not eliminated:
            Aprime, keep = self.eliminate_uncessary_constraints(
                A
            )  # we now do this earlier as well
        ...
        if nnonzero == 0:
            ...
            A2 = UQ[
                :, nnonzero:
            ]  # we need to perform an unconstrained minimization over the smaller A2 to find the information we need.
            ...
            phi_indpt = np.dot(
                Q, yz
            )  # here we reconstruct the full set of symmetric spring constants, which obey ASR
    ...
    def eliminate_uncessary_constraints(self, ASR):
        ...
        if ncons > nvars:
            ...
            depind = np.concatenate(
                (depind, nvars + np.arange(ncons - nvars, dtype=int))
            )
    ...
    def single_rfe(self, U, F, train, test, n_features_target, step=1):
        ...
        while np.sum(support) > n_features_target:
            ...
            self.fit(
                U[train, :], F[train], features
            )  # we do fitting for the features we have
            ...
            support[features[ranks][0:threshold]] = (
                False  # eliminate coefs we no longer are keeping
            )
        ...
        self.fit(
            U[train, :], F[train], features
        )  # we have to do final fitting with our final set of coeffs
        c = self.coef_
    ...
    def run_rfe_cv(self, U, F, step=1, A=None, c=None):
        ...
        cv = KFold(
            n_splits=6, shuffle=True
        )  # shuffle is pretty important if you have limited data for some predictors

        # cross validation
        scores = []

        n_features_target = int(
            round(U.shape[1] * 0.10)
        )  # we keep a minimum of 10% of original features
        #    n_features_target = 2

        for train, test in cv.split(U, F):
            score, coef, support = self.single_rfe(
                U, F, train, test, n_features_target, step
            )
            scores.append(score)
