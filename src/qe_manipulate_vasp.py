    ...
    for line in f:
        ...
        if len(sp) == 0:
            ...
            if born >= 0 and sp[0] == "ion":
                ...
                zeff[born_atom - 1, born, 0:3] = list(map(float, sp[1:]))
                born += 1
                if born >= 3:
                    born = 0
                    if born_atom >= nat:
                ...
                diel[diel_c, :] = list(map(float, sp[0:3]))
                diel_c += 1
                if diel_c >= 3:
                    diel_c = -1
...
def load_output(inputlines):
    ...
    for line in inputlines:
        ...
        if len(sp) == 0:
            continue
        if cell_counter >= 0:
            A[cell_counter, :] = list(map(float, sp[0:3]))
        ...
        #                print stress
            ...
            #            print coords.shape
            coords[atoms_counter_pos, :] = list(map(float, sp[0:3]))
            atoms_counter_pos += 1
            if atoms_counter_pos >= nat:
                atoms_counter_pos = -1

        elif atoms_counter_force >= 0 and len(sp) == 6:
            forces[atoms_counter_force, :] = list(map(float, sp[3:7]))

            # load coords again here
            coords[atoms_counter_force, :] = list(map(float, sp[0:3]))
            coords[atoms_counter_force, :] = (
                coords[atoms_counter_force, :] / 0.52917721067
            )
            coords[atoms_counter_force, :] = np.dot(
                coords[atoms_counter_force, :], np.linalg.inv(A)
            )
...
def load_atomic_pos(fil, return_kpoints=False):
    ...
    A[0, :] = list(map(float, fil[2].split()))
    A[1, :] = list(map(float, fil[3].split()))
    A[2, :] = list(map(float, fil[4].split()))
    ...
    ntypes = list(map(int, fil[6].split()))
    ...
    for n in range(8, 8 + nat):
        coords.append(list(map(float, fil[n].split()[0:3])))
