...
def output_qe_style(filename, pos, A, forces, energy, stress):
    ...
    output.write(
        "a(1) = ( " + str(A[0, 0]) + " " + str(A[0, 1]) + " " + str(A[0, 2]) + " ) \n"
    )
    output.write(
        "a(2) = ( " + str(A[1, 0]) + " " + str(A[1, 1]) + " " + str(A[1, 2]) + " ) \n"
    )
    output.write(
        "a(3) = ( " + str(A[2, 0]) + " " + str(A[2, 1]) + " " + str(A[2, 2]) + " ) \n"
    )
...
def load_output_both(thefile, relax_load_freq=1):
    ...
    if vasp:
        ...
        for (
            ...
            inputlines
        ):  # if we are doing a relaxation, we split the inputfile up into a piece for each relaxation step.
            sp = line.split()
            ...
            if len(sp) == 6 and sp[0] == "number" and sp[2] == "bfgs":
                ...
                splitfiles.append(
                    [nat]
                )  # this is information we need at the beginning of each subfile
                splitfiles[-1].append(celldm)
                splitfiles[-1].append(
                    a1
                )  # a1,a2,a3 are needed if we are doing a fixed cell relaxion. they are overwritten if we are doing a vc-relax
        ...
        if relax_load_freq < 1:  # keep only the last file
            ...
            relax_load_freq > 1 and len(splitfiles) > 5
        ):  # keep a fraction of the files from long relaxations
            s = []
        ...
        if nfile > 1:
            print(
                "number of seperate calcs in this file "
                + str(nfile)
                + " and we use "
                + str(len(splitfiles))
            )
    ...
    #    ENERGIES = []
        ...
        try:
            ...
            if type(thefile) == list:
                ...
                print(
                    "SOMETHING WRONG WITH thefile, not list or string: " + str(thefile)
                )
                inp = open(thefile, "r")
                inputlines = inp.readlines()
                inp.close()
                ...
                if len(sp) > 0:
                    if atoms_counter_pos > -1:
                        ...
                        if crys == False:
                            ...
                            pos[atoms_counter_pos, :] = np.dot(
                                t, np.linalg.inv(A / celldm)
                            )
                        else:
...
def load_atomic_pos(fil, return_kpoints=False):
    ...
    for lines in alllines:
        ...
        if len(sp) < 1:
            ...
            atoms.append(
                sp[0].strip("1").strip("2").strip("3").strip("4").strip("5").strip("6")
            )
...
def generate_random_inputs(
    ...
    if exact_fraction:  # we have to figure out how many dopants exactly
        ...
        if ndope == 0 or ndope == nsites:
            exact_fraction = (
                False  # we don't need to go through the hoopla, existing code will work
            )
    ...
    for c in range(number):
        ...
        if vasp:
            qe_manipulate_vasp.cell_writer(
                coordsnew, Anew, atoms, atoms_big, [1, 1, 1], name
            )
        else:
                ...
                #                out_rev.write("prefix = '"+prefix+str(c+1)+"'\n")
                    ...
                    if theatom != "Vac":
                        out.write(
                            theatom
                            + "  "
                            + str(coordsnew[atom_counter, 0])
                            + " "
                            + str(coordsnew[atom_counter, 1])
                            + " "
                            + str(coordsnew[atom_counter, 2])
                            + "\n"
                        )
            ...
            if dontkeep == False:
                if atom_counter + 1 in keep_list:
                    ...
                    out.write(
                        theatom
                        + "  "
                        + str(coords[atom_counter, 0])
                        + " "
                        + str(coords[atom_counter, 1])
                        + " "
                        + str(coords[atom_counter, 2])
                        + "\n"
                    )
            else:
...
def generate_supercell(fullfil, supercell, outname=None):
    ...
    #    fullfil = fil.readlines()
    #    fil.close()
    A, atoms, coords, coords_type, masses, kpoints = load_atomic_pos(
        fullfil, return_kpoints=True
    )
    ...
    if outname == [] or outname is None:
        ...
        kpoints_super = [
            int(round(float(kpoints[0]) / supercell[0])),
            int(round(float(kpoints[1]) / supercell[1])),
            int(round(float(kpoints[2]) / supercell[2])),
        ]

        cell_writer(
            fullfil, coordsnew, Anew, atoms, coords_type_big, kpoints_super, name
        )
        return coordsnew, Anew, coords_type_big
    ...
    for lines in fullfil:
        ...
        if len(sp) == 1:
            if sp[0].lower() == "direct":
                vasp = True
                qe_manipulate_vasp.cell_writer(
                    coordsnew, Anew, atoms, coords_type, kpoints, name
                )
                return
...
def generate_nondiagonal_supercells(A, kgrid, kpoints):
    ...
    for i in range(nk):
        ...
        denom = np.array(
            [f[0].denominator, f[1].denominator, f[2].denominator], dtype=int
        )
        lcm = int(denom[1] * denom[2] / gcd(denom[1], denom[2]))
        ...
        for i in range(nk):
            ...
            for s11 in range(1, 1 + super_size[i]):
                if super_size[i] % s11 != 0:
                    continue
                quotient = int(super_size[i] / s11)
                for s22 in range(1, 1 + quotient):
                    if quotient % s22 != 0:
                        continue
                    s33 = int(quotient / s22)
                            ...
                            for s23 in range(s33):
                                ...
                                if all(abs(abs(prim) - np.round(abs(prim))) < 1e-7):
                                    ...
                                    for j in range(i + 1, nk):
                                        ...
                                        for k in range(3):
                                            prim[k] = np.sum(
                                                temp_scell[k, :] * kpoints[j, :]
                                            )
                                        if all(
                                            abs(abs(prim) - np.round(abs(prim))) < 1e-7
                                        ):
                                            found_kpoint[j] = 1
                                            label[j] = count
                                    ...
                                    temp_lattice_vectors = minkowski_reduce(
                                        temp_lattice_vectors
                                    )
                                    print("temp latt2")
                                    print(temp_lattice_vectors)
...
def generate_all_supercell_nonorth(inputfile, kgrid):
    ...
    mystruct = Atoms(symbols=coords_type, cell=A, scaled_positions=coords, pbc=True)
    ...
    ):  # include all rotations of unit cell
        print(r)
        print(t)
    ...
    for best, [c1, c2] in zip(BEST, BEST_num):
        ...
        generate_supercell_nonorthog(
            text, inputfile + ".kk." + str(c1) + "." + str(c2), hnf, kgrid, kden
        )
