...
class spring_cluster:
    ...
    def __init__(self, hs_file=None, supercell=[1, 1, 1], outputfile=None):
        ...
        self.relax_load_freq = (
            6  # by default load every 6th structure from a relaxation
        )
    ...
    def set_energy_limit(self, energy=100000000.0):
    ...
    def unfold_input(self, fl, use_input=False):
        ...
        kpoints_super = [
            int(round(float(kpoints[0]) / self.supercell[0])),
            int(round(float(kpoints[1]) / self.supercell[1])),
            int(round(float(kpoints[2]) / self.supercell[2])),
        ]
    ...
    def setup_cutoff(
        self, dim, cutoff=0.01, body=100, dist_cutoff_twobody=0.0001, limit_xy=False
    ):
        ...
        if cutoff < -1e-5:
            if cutoff == -1:
                ...
                print(
                    "WARNING, we only support up to 4th nn as negative integers. setting to 4th n.n. cutoff",
                    dim,
                )
                self.cutoffs[dh] = self.myphi.fourthnn + 1e-5
            else:
                print(
                    "WARNING, setting small negative cutoff to small positive cutoff ",
                    dim,
                )
    ...
    def apply_sym(self, dim):
        ...
        nind, ntotal_ind, ngroups, Tinv, nonzero_list = self.myphi.apply_sym_phi(
            copy.copy(dim),
            self.cutoffs[dh],
            self.bodies[dh],
            dist_cutoff_twobody=self.cutoff_twobody[dh],
            limit_xy=self.limit_xy[dh],
        )
    ...
    def setup_lsq(self, dim):
        ...
        if self.ntotal_ind[dh] == 0:
            ...
            Umat, ASR = self.myphi.setup_lsq_fast(
                self.nind[dh],
                self.ntotal_ind[dh],
                self.ngroups[dh],
                self.trans[dh],
    ...
    def set_regression(
        self, method="lsq", num_keep=0, choose_rfe="good-median", alpha=-1
    ):
        if method.lower() == "rfe" or method.lower() == "recursive feature elimination":
            ...
            if self.myphi.num_keep > 1:
                print(
                    "Using recursive feature elimination with ",
                    self.myphi.num_keep,
                    " features",
                )
            else:
    ...
    def do_lsq(self):
        ...
        UMAT = np.zeros(
            (self.Umat[self.dim_hash(self.dims[0])].shape[0], ctot), dtype=float
        )
        ...
        for d, ra, ca in zip(self.dims, r, c):
            ...
            self.nz[dh], self.phi_nz[dh] = (
                self.myphi.reconstruct_fcs_nonzero_phi_relative(
                    phi_ind_dim,
                    self.ngroups[dh],
                    self.nind[dh],
                    self.trans[dh],
    ...
    def do_all_fitting(self):
        ...
        if self.verbosity == "High" or self.myphi.verbosity == "High":
            print(
                "LSQ Timing (not accurate for multiple processors) "
                + str(time2 - time1)
            )
    ...
    def write_harmonic(self, filename, dontwrite=False):
        ...
        if not 2 in self.dims_hashed:
            ...
            string = self.myphi.write_harmonic_qe(
                filename,
                self.phi_nz[2],
                self.nz[2],
                self.supercell,
    ...
    def write_cubic(self, filename, write=True):
        ...
        if not 3 in self.dims_hashed:
            ...
            cubic = self.myphi.write_thirdorder_shengbte(
                filename, self.phi_nz[3], self.nz[3]
            )
        #      print 'take 2'
    ...
    def calc_energy(self, A, pos, types, cell=[]):
        ...
        energy, forces, stress = self.myphi.calculate_energy_force_stress(
            A, pos, types, self.dims, [], phis, nzs, cell
        )
    ...
    def run_mc_test(self, A, pos, types, cell=[]):
        ...
        starting_energy = self.myphi.run_montecarlo_test(
            A,
            pos,
            types,
            self.dims,
    ...
    def calc_energy_qe_file(self, filename, cell=[], ref=None):
        ...
        A, types, pos, forces, stress, energy = load_output(
            filename
        )  # only gets the last entry from a relaxation
        energy, forces, stress, energy_ref, forces_ref, stress_ref = (
            self.calc_energy_qe_output(
                A, types, pos, forces, stress, energy, cell=cell, ref=ref
            )
        )

        return energy, forces, stress, energy_ref, forces_ref, stress_ref

    def calc_energy_qe_output(
        self, A, types, pos, forces_ref, stress_ref, energy_ref, cell=[], ref=None
    ):
        ...
        if (
            not (isinstance(energy_ref, int) or isinstance(energy_ref, float))
            or abs(energy_ref - -99999999) < 1e-5
        ):
        ...
        (
            ...
            A, types, pos, bestfitcell, forces_ref, stress_ref, energy_ref
        )
        ...
        A, strain, rotmat, forces_ref, stress_ref = self.myphi.get_rot_strain(
            A, refA, forces_ref, stress_ref
        )
        ...
        if self.myphi.vacancy != 1:
            ntypes += (
                abs(np.linalg.det(bestfitcell)) * self.myphi.nat - pos.shape[0]
            )  # for doping
        ...
        energy_ref = energy_ref - self.myphi.energy_ref * abs(
            np.linalg.det(bestfitcell)
        )
    ...
    def calc_energy_qe_output_list(self, tocalc):
        ...
        if type(tocalc) is list:  # we have a list of other files already
            ...
            type(tocalc) is str
        ):  # we have the name of a file with a list of other files
            fil = open(tocalc, "r")
            f = fil.readlines()
            fil.close()
        else:
        ...
        for line in f:
            ...
            if len(ls) == 3:
                ...
                correction = (
                    energy1 - self.myphi.energy_ref * pos1.shape[0] / self.myphi.nat
                )
            ...
            bestfitcell_input = []
            if len(ls) == 5:
                bestfitcell_input = list(map(int, ls[2:5]))
                print(
                    "bestfitcell_input (calc_energy_qe_output_list): ",
                    bestfitcell_input,
                )
            ...
            A_big, types_big, pos_big, forces_big, stress_big, energy_big = (
                load_output_both(filename, relax_load_freq=self.relax_load_freq)
            )
            if A_big is None:
                continue

            for c, [A, types, pos, forces, stress, energy_ref] in enumerate(
                ...
                if (
                    ...
                    print(
                        "ERROR, Failed to load " + str(ls[0]) + ", number ",
                        c,
                ...
                energy_ref = (
                    energy_ref - correction * np.array(forces).shape[0] / pos.shape[0]
                )
                ...
                print(
                    "F abs "
                    + str(np.sum(np.sum(abs(forces_ref - forces))) / (nat * 3.0))
                    + " "
                    + str(np.sum(np.sum(abs(forces_ref))) / (nat * 3.0))
                    + "\t \t"
                    + str(filename)
                    + " "
                    + str(c)
                ...
                print(
                    "F max abs "
                    + str(np.max(np.max(abs(forces_ref - forces))))
                    + " "
                    + str(np.max(np.max(abs(forces_ref))))
                    + "\t \t"
                    + str(filename)
                    + " "
                    + str(c)
                ...
                print(
                    "S abs "
                    + str(np.sum(np.sum(abs(stress_ref - stress))))
                    + " "
                    + str(np.sum(np.sum(abs(stress_ref))))
                    + "\t \t"
                    + str(filename)
                    + " "
                    + str(c)
                ...
                Eref += abs(energy_ref)
                dFtot += np.sum(np.sum(np.abs(forces - forces_ref))) / (
                    3 * forces.shape[0]
                )
    ...
    def calc_energy_qe_input_list(self, filename):
        ...
        for line in f:
            ...
            energy, forces, stress = self.calc_energy_qe_input(
                ls[0], cell=bestfitcell_input, ref=ref
            )
    ...
    def calc_energy_qe_input(self, filename, cell=[], ref=None):
        ...
        (
            ...
            refpos,
            bf,
        ) = self.myphi.unfold(A, types, pos, bestfitcell)
        bestfitcell = np.diag(bestfitcell)
    ...
    def plot_comparison(self, A, B, filename="plt.pdf", show=False):
        ...
        scale = max(list(map(abs, [amin, bmin, amax, bmax])))
        s = scale * 0.03
    ...
    def Fvib_freq(self, qpoints, T):
        ...
        self.myphi.load_harmonic_new(
            harmonicstring, False, zero=False, stringinput=True
        )
        ...
        for freq in FREQ:
            ...
            for f in freq:
                ...
                if f > 10.000:  # need to cut out acoustic modes
                    Fvib += hbar_cm * f / 2.0 + kbT * np.log(
                        1 - np.exp(-hbar_cm * f / kbT)
                    )
                    t = hbar_cm * f / kbT
        ...
        if nskip != 3 and nskip != 0:
            ...
            print(
                "Should be only the 3 acoustic modes if grid includes Gamma unless your kpoint sampling is very high. You may have unstable modes"
            )
        ...
        for i in range(DOS.shape[0]):
            if DOS[i, 0] > 10.000:
                Fvibdos += (
                    hbar_cm * DOS[i, 0] / 2
                    + kbT * np.log(1 - np.exp(-hbar_cm * DOS[i, 0] / kbT))
                ) * DOS[i, 1]
    ...
    def phonon_band_structure(
        ...
        qpoints,
        nsteps=20,
        filename="bandstruct.csv",
        filename_plt="bandstruct.pdf",
        show=False,
    ):
    ...
    def gruneisen_total(self, qpoints, T):
        ...
        qpts = self.myphi.dyn.generate_qpoints_simple(
            qpoints[0], qpoints[1], qpoints[2]
        )
        ...
        for nq in range(qpts.shape[0]):
            ...
            gr[:], ev2[:] = self.gruneisen_fast(
                qpts[nq, :], R_big, dphi_big, singlepoint=False
            )
            ev[:] = abs(ev2) ** 0.5
            x[:] = ev / (2.0 * kbT)
            if all(ev2 > 0.8e-7):
    ...
    def relax(self, A, pos, types, relax_unit_cell=True):
        ...
        strain_constant = 50.0  # BFGS relaxation takes gigantic initial strain steps if you don't use a constant. i do not understand why...
        forces_constant = 5.0
        counter = 0
        ...
        u, types_s, u_super, supercell = self.myphi.figure_out_corresponding(
            pos, A, types
        )
        ...
        if relax_unit_cell == False:
            ...
            crys_final = self.myphi.coords_super + np.dot(
                u_final, np.linalg.inv(A_final)
            )
        ...
        for mc_step in range(0 + mc_start, steps + mc_start):
            ...
            try:
                ...
                for e, f, er, fr in zip(e, f, er, fr):
                    ...
                    print(
                        "Forces PREDICTION ",
                        np.max(np.abs(f.flatten() - fr.flatten())),
                        np.max(np.abs(fr.flatten())),
                    )
            ...
            ncalc_new = self.load_filelist(
                newfile, add=True
            )  # we add to file list instead of overwriting to save time
            if ncalc_new > 0:
                self.do_all_fitting()
