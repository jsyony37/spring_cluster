...
class dyn:
    ...
    def generate_qpoints_simple(self, N1, N2, N3):
        ...
        if self.verbosity == "High":
            print(
                "generating " + str(N1) + " X " + str(N2) + " X " + str(N3) + " grid "
            )
    ...
    def calc_kappa(self, N, NN, smear, nstep, temp, dplus, dminus):
        ...
        p = (
            (1 - 0.514 * gamma**-1 + 0.228 * gamma**-2)
            / 0.0948
            * self.hbar**2
            * gamma2
            / (self.kb * debye * self.MASS * V ** (1.0 / 3.0) * s)
        )
        ...
        for f, v in zip(EVALS, VELS):
            ...
            C = np.array([self.Ciq(x, temp) for x in omega], dtype=float)
    ...
    def dos(self, N, smear, nstep, velocity):
        ...
        for step in range(nstep):
            ...
            g = np.sum([self.gaussian(x, e, smear) for x in freq]) * wq
    ...
    def debyeT(self, DOS):
        ...
        for [e, d] in DOS:
            ...
            # convert to radians!!!!!!!!!!!
        conversion_factor_cm_sec = (
            1.0 / self.ryd_to_cm**2 * self.ryd_to_thz**2 * 10**24 * (2.0 * np.pi) ** 2
        )

        prefactor = (
            float(self.nat) ** (-1.0 / 3.0)
            * (5 * self.hbar**2 / (3 * self.kb**2)) ** 0.5
        )
        ...
        print(
            ...
            + str(int2)
            + " cm2 "
            + str(conversion_factor_cm_sec * int2)
            + " sec2"
        )
    ...
    def load_harmonic(
        ...
        if (
            filename is None
        ):  # do not load from a file, take input from other parameters
            ...
            if (
                ...
                print(
                    "error dynmat load_harmonic need to include A dielectric zeff coords types if not loading file"
                )
                print(A)
                print(dielectric)
                print(zeff)
            ...
            if sp[0][0:4] == "vasp":
                ...
                vasp = True
                ntype, nat, A, coords, names_dict, type_nums, zeff_list, diel = (
                    qe_manipulate_vasp.load_diel(f.readlines())
                )
        ...
        if vasp == False:
            ...
            self.celldm = list(map(float, sp[3:]))
            if self.brav == 0:
                line = f.readline()
                sp = line.split()
                self.A[0][0:3] = list(map(float, sp))
                line = f.readline()
                sp = line.split()
                self.A[1][0:3] = list(map(float, sp))
                line = f.readline()
                sp = line.split()
                self.A[2][0:3] = list(map(float, sp))
                #            print self.A
                self.A = self.A
            elif self.brav == 2:
            ...
            if self.verbosity == "High":
                ...
                for a in range(0, self.nat):
                    print(
                        str(self.names[a][0])
                        + " "
                        + str(self.dict[self.names[a][1]][0])
                        + " "
                        + str(self.dict[self.names[a][1]][1])
                        + " "
                        + str(self.pos[a][0])
                        + " "
                        + str(self.pos[a][1])
        ...
        for line in f:
            ...
            #                print d1 + ' ' + d2 + ' ' + a1 + ' ' + a2
                ...
                if self.verbosity == "High":
                    ...
                    print(float(sp[3]))
                self.harm[
                    xyz[0] * self.R[1] * self.R[2] + xyz[1] * self.R[2] + xyz[2],
                    (a1 - 1) * 3 + d1 - 1,
                    (a2 - 1) * 3 + d2 - 1,
                ] = float(sp[3])
            ...
            for n2 in range(-2 * self.R[1], 2 * self.R[1] + 1):
                for n3 in range(-2 * self.R[2], 2 * self.R[2] + 1):
                    ...
                    for a in range(self.nat):
                        Ara[a, :] = np.dot(
                            np.array([n1, n2, n3]) + 0 * self.pos_crys[a, :], self.Areal
                        )
                        for b in range(self.nat):
                            Arb = np.dot(
                                np.array([n1, n2, n3]) + 0 * self.pos_crys[b, :],
                                self.Areal,
                            )
                    ...
                    if t:
                        ...
                        for na in range(self.nat):
                            for nb in range(self.nat):
                                ...
                                for i in range(3):
                                    for j in range(3):
                                        self.harm_ws[c, 3 * na + i, 3 * nb + j] = (
                                            w * self.harm[index, 3 * na + i, 3 * nb + j]
                                        )
        ...
        for d1 in range(3):
            for d2 in range(3):
                for a1 in range(self.nat):
                    ...
                    self.harm[0, (a1) * 3 + d1, (a1) * 3 + d2] = (
                        self.harm[0, (a1) * 3 + d1, (a1) * 3 + d2] - sum_tot
                    )
    ...
    def asr_crystal(self):
                            ...
                            for n2 in range(self.R[1]):
                                for n3 in range(self.R[2]):
                                    ...
                                    for l in range(m):
                                        if (
                                            ind_v[l, 0, 0] == index[0]
                                            and ind_v[l, 0, 1] == index[1]
                                            and ind_v[l, 0, 2] == index[2]
                                        ):
                                            q = 0
                                            break
                                        if (
                                            ind_v[l, 1, 0] == index[0]
                                            and ind_v[l, 1, 1] == index[1]
                                            and ind_v[l, 1, 2] == index[2]
                                        ):
                                            q = 0
                                            break

                                    if (
                                        n1 == (self.R[0] - n1) % self.R[0]
                                        and n2 == (self.R[1] - n2) % self.R[1]
                                        and n3 == (self.R[2] - n3) % self.R[2]
                                        and i == j
                                        and na == nb
                                    ):
                                        q = 0
        ...
        for k in range(p):
            ...
            for l in range(m):
                ...
                #               print 'scal ' + str(scal)
                for r in range(2):
                    w[ind_v[l, r, 0], ind_v[l, r, 1], ind_v[l, r, 2]] += (
                        -1.0 * scal * v[l, r]
                    )
        ...
        for j in range(3):
            for nb in range(self.nat):
                for nr in range(self.R[0] * self.R[1] * self.R[2]):
                    scal += (
                        u[nr, na * 3 + i, nb * 3 + j] * v[nr, 3 * na + i, 3 * nb + j]
                    )
    ...
    def integrate_grun2(self, dplus, dminus, temp, debyeT, N):
        ...
        for f, gr in zip(freq, grun):
            # rad per sec
            omega = [
                x * 2.0 * np.pi / self.ryd_to_cm * self.ryd_to_thz * 10**12 for x in f
            ]
            C = [self.Ciq(x, temp) for x in omega]
    ...
    def get_hk_with_derivatives(self, k, coords_crys):
        ...
        for a in range(self.nat * 3):
            for b in range(self.nat * 3):
                for i in range(3):
                    ...
                    for j in range(3):
                        hk_prime_prime[a, b, i, j] = -np.sum(
                            self.RR_ws[:, i, j] * t[:, a, b], 0
                        )
        ...
        for n1 in range(self.nat):
            for n2 in range(self.nat):
                ...
                DX = np.dot(
                    np.array(coords_crys[n1]) - np.array(coords_crys[n2]), self.Areal
                )

                arg = -2.0 * np.pi * (np.dot(k, dx)) * 1j
                hk3[n1 * 3 : n1 * 3 + 3, n2 * 3 : n2 * 3 + 3] *= np.exp(arg)
                temp = copy.copy(
                    hk_prime[n1 * 3 : n1 * 3 + 3, n2 * 3 : n2 * 3 + 3, :] * np.exp(arg)
                )
                #                print 'dx temp n1 n2 ' + str(dx) + ' ' + str(temp) + ' ' + str([n1,n2]) + ' ' + str(np.outer(dx,temp))
                for i in range(3):
                    for j in range(3):
                        for ii in range(3):
                            hk_prime[n1 * 3 + i, n2 * 3 + j, ii] = (
                                hk_prime[n1 * 3 + i, n2 * 3 + j, ii] * np.exp(arg)
                                + -1j * DX[ii] * hk3[n1 * 3 + i, n2 * 3 + j]
                            )  #
                            for jj in range(3):
                                qa[n1 * 3 + i, n2 * 3 + j, ii, jj] += hk_prime_prime[
                                    n1 * 3 + i, n2 * 3 + j, ii, jj
                                ] * np.exp(arg)
                                qax[n1 * 3 + i, n2 * 3 + j, ii, jj] += np.exp(arg)
                                qay[n1 * 3 + i, n2 * 3 + j, ii, jj] += (
                                    temp[i, j, ii] * -1j * DX[jj]
                                    + temp[i, j, jj] * -1j * DX[ii]
                                )

                                hk_prime_prime[n1 * 3 + i, n2 * 3 + j, ii, jj] = (
                                    hk_prime_prime[n1 * 3 + i, n2 * 3 + j, ii, jj]
                                    * np.exp(arg)
                                    - hk3[n1 * 3 + i, n2 * 3 + j] * DX[ii] * DX[jj]
                                    + temp[i, j, ii] * -1j * DX[jj]
                                    + temp[i, j, jj] * -1j * DX[ii]
                                )
                                q[n1 * 3 + i, n2 * 3 + j, ii, jj] += (
                                    -hk3[n1 * 3 + i, n2 * 3 + j] * DX[ii] * DX[jj]
                                )
        ...
        if self.nonan:
            ...
            #            hk3 += nonan_only
            hktemp = np.zeros((self.nat * 3, self.nat * 3), dtype=complex)
            nonan_only, nonan_prime, npp = self.add_nonan_with_derivative(
                hktemp, k, coords_crys
            )
            hk3 += nonan_only
        #            hk3 = nonan_only
            ...
            for i in range(3):
                for j in range(3):
                    for na in range(self.nat):
                        for nb in range(self.nat):
                            for ii in range(3):
                                hk_prime[na * 3 + i, nb * 3 + j, ii] += nonan_prime[
                                    na * 3 + i, nb * 3 + j, ii
                                ] / (2 * np.pi)
                                #                                print ['p',hk_prime[na*3+i,nb*3+j, ii],na,nb,i,j,ii]
                                for jj in range(3):
                                    hk_prime_prime[na * 3 + i, nb * 3 + j, ii, jj] += (
                                        npp[na * 3 + i, nb * 3 + j, ii, jj]
                                        / (2 * np.pi) ** 2
                                    )
        ...
        for k in klist:
            ...
            if velocity == True:
                ...
                hk_dq = (
                    ...
                    * np.array(
                        [
                            np.sum(arg_R[0] * self.harm_ws, 0),
                            np.sum(arg_R[1] * self.harm_ws, 0),
                            np.sum(arg_R[2] * self.harm_ws, 0),
                        ]
                    )
                )
                hk_dq[0] = (
                    ((hk_dq[0] + np.conjugate(hk_dq[0].transpose())) / 2.0)
                    * self.massmat
                    * self.ryd_to_hz**2
                )
                hk_dq[1] = (
                    ((hk_dq[1] + np.conjugate(hk_dq[1].transpose())) / 2.0)
                    * self.massmat
                    * self.ryd_to_hz**2
                )
                hk_dq[2] = (
                    ((hk_dq[2] + np.conjugate(hk_dq[2].transpose())) / 2.0)
                    * self.massmat
                    * self.ryd_to_hz**2
                )
                    ...
                    for a, t in zip(self.names, coords_small):
                        ...
                        if self.verbosity == "High":
                            print(
                                self.dict[a[1]][0]
                                + " "
                                + str(t[0] / N[0] + float(x) / N[0])
                                + " "
                                + str(t[1] / N[1] + float(y) / N[1])
                                + " "
                                + str(t[2] / N[2] + float(z) / N[2])
                            )
                        types.append(self.dict[a[1]][0])
                        coords[c, :] = t / np.array(N, dtype=float) + np.array(
                            [x, y, z], dtype=float
                    ...
                    if geg > 0.0 and geg / alph / 4.0 < gmax:
                        ...
                        for na in range(self.nat):
                            ...
                            for i in range(0, 3):
                                for j in range(0, 3):
                                    hk[na * 3 + i, na * 3 + j] += (
                                        -1 * facgd * zag[i] * fnat[j]
                                    )  # np.exp(2.0*np.pi*np.dot(k,(np.array(self.pos[na],dtype=float)-np.array(self.pos[nb],dtype=float))))
                            ...
                            for na in range(self.nat):
                                ...
                                for i in range(0, 3):
                                    for j in range(0, 3):
                                        hk[3 * na + i, 3 * nb + j] += (
                                            facg * zag[i] * zbg[j]
                                        )  # np.exp(1.0j*2.0*np.pi*np.dot(k,(np.array(self.pos[na],dtype=float)-np.array(self.pos[nb],dtype=float))))
    ...
    def add_nonan_with_derivative(self, hk, k, coords_crys):
            ...
            for m2 in range(-nr2x, nr2x + 1):
                for m3 in range(-nr3x, nr3x + 1):
                            ...
                            for nb in range(self.nat):
                                ...
                                for i in range(0, 3):
                                    for j in range(0, 3):
                                        ...
                                        hk[na * 3 + i, na * 3 + j] += (
                                            -1 * facgd * zag[i] * fnat[j]
                                        )  # * np.exp(2.0*np.pi*np.dot(g,(np.array(self.pos[na],dtype=float)-np.array(self.pos[nb],dtype=float))))
                    ...
                    if geg > 1e-5 and geg / alph / 4.0 < gmax:
                        ...
                        geg_prime[0] = np.dot(
                            np.dot(dgx, self.eps), g.transpose()
                        ) + np.dot(np.dot(g, self.eps), dgx.transpose())
                        geg_prime[1] = np.dot(
                            np.dot(dgy, self.eps), g.transpose()
                        ) + np.dot(np.dot(g, self.eps), dgy.transpose())
                        geg_prime[2] = np.dot(
                            np.dot(dgz, self.eps), g.transpose()
                        ) + np.dot(np.dot(g, self.eps), dgz.transpose())
                        ...
                        geg_prime_prime[0, 0] = np.dot(
                            np.dot(dgx, self.eps), dgx.transpose()
                        ) + np.dot(np.dot(dgx, self.eps), dgx.transpose())
                        geg_prime_prime[0, 1] = np.dot(
                            np.dot(dgx, self.eps), dgy.transpose()
                        ) + np.dot(np.dot(dgy, self.eps), dgx.transpose())
                        geg_prime_prime[0, 2] = np.dot(
                            np.dot(dgx, self.eps), dgz.transpose()
                        ) + np.dot(np.dot(dgz, self.eps), dgx.transpose())

                        geg_prime_prime[1, 0] = np.dot(
                            np.dot(dgy, self.eps), dgx.transpose()
                        ) + np.dot(np.dot(dgx, self.eps), dgy.transpose())
                        geg_prime_prime[1, 1] = np.dot(
                            np.dot(dgy, self.eps), dgy.transpose()
                        ) + np.dot(np.dot(dgy, self.eps), dgy.transpose())
                        geg_prime_prime[1, 2] = np.dot(
                            np.dot(dgy, self.eps), dgz.transpose()
                        ) + np.dot(np.dot(dgz, self.eps), dgy.transpose())

                        geg_prime_prime[2, 0] = np.dot(
                            np.dot(dgz, self.eps), dgx.transpose()
                        ) + np.dot(np.dot(dgx, self.eps), dgz.transpose())
                        geg_prime_prime[2, 1] = np.dot(
                            np.dot(dgz, self.eps), dgy.transpose()
                        ) + np.dot(np.dot(dgy, self.eps), dgz.transpose())
                        geg_prime_prime[2, 2] = np.dot(
                            np.dot(dgz, self.eps), dgz.transpose()
                        ) + np.dot(np.dot(dgz, self.eps), dgz.transpose())

                        facgd = fac * np.exp(-geg / alph / 4.0) / geg
                        #                        facgd_p = fac * (-np.exp(-geg / alph / 4.0)/geg**2 * geg_prime + -geg_prime/alph/4.0 * np.exp(-geg / alph / 4.0)/geg)
                        facgd_p = fac * (
                            -np.exp(-geg / alph / 4.0) / geg**2 * geg_prime
                            + -geg_prime / alph / 4.0 * np.exp(-geg / alph / 4.0) / geg
                        )
                        #                        facgd_p = fac * (-np.exp(-geg / alph / 4.0)/geg**2 * geg_prime + -geg_prime/alph/4.0 * np.exp(-geg / alph / 4.0)/geg)

                        facgd_p_p = fac * (
                            -np.exp(-geg / alph / 4.0) / geg**2 * geg_prime_prime
                            + -geg_prime_prime
                            / alph
                            / 4.0
                            * np.exp(-geg / alph / 4.0)
                        ...
                        facgd_p_p += fac * (
                            np.exp(-geg / alph / 4.0) / geg**2 / 4.0 / alph * gg
                            + 2.0 * np.exp(-geg / alph / 4.0) / geg**3 * gg
                        )
                        facgd_p_p += fac * (
                            gg / alph**2 / 4.0**2 * np.exp(-geg / alph / 4.0) / geg
                            + gg / alph / 4.0 * np.exp(-geg / alph / 4.0) / geg**2
                        )
                        ...
                        for nb in range(self.nat):
                            ...
                            for na in range(self.nat):
                                ...
                                for i in range(3):
                                    for j in range(3):
                                        dzdz[:, :, i, j] = (
                                            np.outer(dzbg[j, :], dzag[i, :])
                                            + np.outer(dzag[i, :], dzbg[j, :])
                                        ) / 2.0
                                ...
                                facgd_p_dx = np.outer(dxB, facgd_p) + np.outer(
                                    facgd_p, dxB
                                )

                                facg_prime = facgd * (
                                    2.0 * np.pi * 1.0j * dxB
                                ) * np.exp(1.0j * arg) + facgd_p[:] * np.exp(1.0j * arg)
                                ...
                                for i in range(0, 3):
                                    for j in range(0, 3):
                                        ...
                                        hk_prime_t[3 * na + i, 3 * nb + j, 0] += (
                                            facg_prime[0] * zag[i] * zbg[j] * exp
                                        )
                                        hk_prime_t[3 * na + i, 3 * nb + j, 1] += (
                                            facg_prime[1] * zag[i] * zbg[j] * exp
                                        )
                                        hk_prime_t[3 * na + i, 3 * nb + j, 2] += (
                                            facg_prime[2] * zag[i] * zbg[j] * exp
                                        )

                                        hk_prime_t[3 * na + i, 3 * nb + j, 0] += (
                                            (dzag[i, 0] * zbg[j] + zag[i] * dzbg[j, 0])
                                            * facg
                                            * exp
                                        )
                                        hk_prime_t[3 * na + i, 3 * nb + j, 1] += (
                                            (dzag[i, 1] * zbg[j] + zag[i] * dzbg[j, 1])
                                            * facg
                                            * exp
                                        )
                                        hk_prime_t[3 * na + i, 3 * nb + j, 2] += (
                                            (dzag[i, 2] * zbg[j] + zag[i] * dzbg[j, 2])
                                            * facg
                                            * exp
                                        )

                                        hk_prime_t[3 * na + i, 3 * nb + j, 0] += (
                                            zag[i] * zbg[j] * facg * dexp[0]
                                        )
                                        hk_prime_t[3 * na + i, 3 * nb + j, 1] += (
                                            zag[i] * zbg[j] * facg * dexp[1]
                                        )
                                        hk_prime_t[3 * na + i, 3 * nb + j, 2] += (
                                            zag[i] * zbg[j] * facg * dexp[2]
                                        )
                                        ...
                                        for ii in range(3):
                                            for jj in range(3):
                                                ...
                                                hk_prime_prime_t[
                                                    ...
                                                    facg_p_p[ii, jj] * zag[i] * zbg[j]
                                                ) * exp
                                                hk_prime_prime_t[
                                                    3 * na + i, 3 * nb + j, ii, jj
                                                ] += (
                                                    facg * dzdz[ii, jj, i, j] * 2.0
                                                ) * exp
                                                hk_prime_prime_t[
                                                    3 * na + i, 3 * nb + j, ii, jj
                                                ] += (
                                                    (
                                                        np.outer(facg_prime, dzbg[j, :])
                                                        + np.outer(
                                                            dzbg[j, :], facg_prime
                                                        )
                                                    )[ii, jj]
                                                ...
                                                hk_prime_prime_t[
                                                    ...
                                                    * zbg[j]
                                                    * (
                                                        np.outer(dexp, dzag[i, :])[
                                                            ii, jj
                                                        ]
                                                        + np.outer(dzag[i, :], dexp)[
                                                            ii, jj
                                                        ]
                                                    )
                                                )
                                                hk_prime_prime_t[
                                                    ...
                                                    * zag[i]
                                                    * (
                                                        np.outer(dexp, dzbg[j, :])[
                                                            ii, jj
                                                        ]
                                                        + np.outer(dzbg[j, :], dexp)[
                                                            ii, jj
                                                        ]
                                                    )
                                                )
                                                hk_prime_prime_t[
                                                    ...
                                                    (
                                                        np.outer(facg_prime, dexp)
                                                        + np.outer(dexp, facg_prime)
                                                    )[ii, jj]
                                                    * zag[i]
                                                    * zbg[j]
                                                )
        ...
        for nb in range(self.nat):
            for na in range(self.nat):
                ...
                hk_prime[3 * na : 3 * na + 3, 3 * nb : 3 * nb + 3, 0] += (
                    hk_prime_t[3 * na : 3 * na + 3, 3 * nb : 3 * nb + 3, 0]
                    * self.celldm0
                )
                hk_prime[3 * na : 3 * na + 3, 3 * nb : 3 * nb + 3, 1] += (
                    hk_prime_t[3 * na : 3 * na + 3, 3 * nb : 3 * nb + 3, 1]
                    * self.celldm0
                )
                hk_prime[3 * na : 3 * na + 3, 3 * nb : 3 * nb + 3, 2] += (
                    hk_prime_t[3 * na : 3 * na + 3, 3 * nb : 3 * nb + 3, 2]
                    * self.celldm0
                )
            ...
            for k2 in range(self.nat):
                ...
                for alpha in range(3):
                    for beta in range(3):
                        ...
                        Cqx[k1 * 3 + alpha, k2 * 3 + beta] = (
                            e2 * 4.0 * np.pi / self.vol * qz1[alpha] * qz2[beta] / qeq
                        )
        return Cqx
    ...
    def wsinit(self):
        ...
        self.atws = self.A * np.array(
            [
                [self.R[0], self.R[0], self.R[0]],
    ...
    def make_wscache(self):
        self.wscache = np.zeros(
            (self.R[0] * 5, self.R[1] * 5, self.R[2] * 5, self.nat, self.nat),
            dtype=float,
        )
    ...
    def extract_supercell(self, f):
        ...
        h = float(sp[4])
        PHI = np.zeros(
            self.ra, self.ra, self.nat * 3, self.nat * 3, self.nat * 3, dtype=float
        )
        ...
        for line in input:
            ...
            if len(sp) > 0:
                if forcescounter > -1 and forcescounter < nat:
                    ...
                    forces[x, y, z, atom, 0:3] = list(map(float, sp[6]))
                    forcescounter += 1
        ...
        for line in input:
            ...
            if len(sp) > 0:
                if atoms_counter > -1 and atoms_counter < nat:
                    t = np.array(
                        [float(sp[6]), float(sp[7]), float(sp[8])], dtype=float
                    )
                ...
                if sp[0] == "a(1)":
                    A[0, :] = celldm * np.array(
                        [float(sp[3]), float(sp[4]), float(sp[5])], dtype=float
                    )
                if sp[0] == "a(2)":
                    A[1, :] = celldm * np.array(
                        [float(sp[3]), float(sp[4]), float(sp[5])], dtype=float
                ...
                if forces_counter > -1 and forces_counter < nat and len(sp) == 9:
                    ...
                    forces[forces_counter, 0:3] = list(map(float, sp[6:9]))
                    forces_counter += 1
                if sp[0] == "number" and sp[2] == "atoms/cell":
                    nat = int(sp[4])
                    forces = np.zeros((nat, 3), dtype=float)
                ...
                if stress_counter > -1 and stress_counter < 3:
                    stress[stress_counter, 0:3] = list(map(float, sp[0:3]))
                    stress_counter += 1
        ...
        for line in input:
            ...
            if len(sp) > 0:
                ...
                if CELL_PARAMETERS > -1 and CELL_PARAMETERS < 3:
                    A[CELL_PARAMETERS, :] = np.array(
                        [float(sp[0]), float(sp[1]), float(sp[2])], dtype=float
                    )
                    CELL_PARAMETERS += 1
                if sp[0] == "CELL_PARAMETERS":
                    CELL_PARAMETERS = 0
                if ATOMIC_POS > -1 and ATOMIC_POS < nat:
                    coords[ATOMIC_POS, :] = np.array(
                        [float(sp[1]), float(sp[2]), float(sp[3])], dtype=float
                    )
                    ATOMIC_POS += 1
    ...
    def load_atomic_pos(self, fil):
        ...
        for lines in fil:
            ...
            if len(sp) < 1:
                ...
                coords.append(list(map(float, sp[1:4])))
            elif aflag == 1:
                A.append(list(map(float, sp)))
    ...
    def identify_atoms(self, A, coords):
        ...
        x = max(
            list(
                map(
                    abs,
                    list(map(round, A[0, :] / (1e-5 + self.A[0, :] * self.celldm0))),
                )
            )
        )
        y = max(
            list(
                map(
                    abs,
                    list(map(round, A[1, :] / (1e-5 + self.A[1, :] * self.celldm0))),
                )
            )
        )
        z = max(
            list(
                map(
                    abs,
                    list(map(round, A[2, :] / (1e-5 + self.A[2, :] * self.celldm0))),
                )
            )
        )
        ss_dim = [x, y, z]
        ...
        for n1 in range(nat_ss):
            ...
            for n2 in range(nat_ss):
                ...
                if count == 1:  # found exactly 1 copy, find the phi
                    ...
                    #                    print self.celldm0
                    tau = np.dot(
                        self.pos[a2, :] - self.pos[a1, :], self.A * self.celldm0
                    )
                    #                    print self.pos[a2,:]
                    #                    print self.pos[a1,:]
                    #                    print tau
                    ...
                    R = list(map(round, np.dot(RRR - tau, Ainv))) % np.array(self.R)
                    #                    print R
                    #                    print 'asdf2'
                    ...
                    for i in range(3):
                        for j in range(3):
                            big_harm[n1, n2, i, j] = self.harm[
                                R[0] * self.R[1] * self.R[2] + R[1] * self.R[2] + R[2],
                                a2 * 3 + i,
                                a1 * 3 + j,
                            ]
                            ...
                            for i1 in [-1, 0, 1]:
                                for i2 in [-1, 0, 1]:
                                    for i3 in [-1, 0, 1]:
                                        ...
                                        A1a = np.dot(
                                            self.pos_crys[a1, :] + xyz + mod, self.Areal
                                        )
    ...
    def supercell_fourier(self, A, coords):
        ...
        unitcells, R_big, U_crys, U_bohr, Q, big_nat, CODE = self.identify_atoms_harm(
            A, coords
        )
        ...
        for nq in range(unitcells):
            for nr in range(unitcells):
                ...
                F_R_nonan[nr, :, :] += (
                    np.exp((1.0j) * 2 * np.pi * qr) * F_Q_nonan[nq, :, :]
                )
        #
    ...
    def supercell_fourier_make_force_constants(self, A, coords):
        ...
        for nq in range(unitcells):
            for nr in range(unitcells):
                ...
                H_R[nr, :, :] += (
                    np.exp((1.0j) * 2 * np.pi * qr) * H_Q[nq, :, :] / unitcells
                )
        #
        ...
        for x in range(unitcells):
            for y in range(unitcells):
                ...
                for c in range(unitcells):
                    if (
                        R_combo[0] == R_big[c, 0]
                        and R_combo[1] == R_big[c, 1]
                        and R_combo[2] == R_big[c, 2]
                    ):
                        CODE2[x, y] = c
        ...
        for n1 in range(big_nat):
            ...
            for n2 in range(big_nat):
                ...
                harm_normal[n1 * 3 : (n1 + 1) * 3, n2 * 3 : (n2 + 1) * 3] = H_R[
                    CODE2[uc_1, uc_2],
                    atoms_1 * 3 : (atoms_1 + 1) * 3,
                    atoms_2 * 3 : (atoms_2 + 1) * 3,
                ].real
    ...
    def supercell_fourier_make_force_constants_derivative(self, A, coords):
        ...
        for nq in range(unitcells):
            q = Q[nq, :]
            hk, hk2, nonan, npa, npp, hkp, hkpp = (
                self.get_hk_with_derivatives_nonanonly(q, coords)
            )
        ...
        for nq in range(unitcells):
            for nr in range(unitcells):
                ...
                H_Rp[nr, :, :, :] += (
                    np.exp((1.0j) * 2 * np.pi * qr) * H_Qp[nq, :, :, :] / unitcells
                )
                H_Rpp[nr, :, :, :, :] += (
                    np.exp((1.0j) * 2 * np.pi * qr) * H_Qpp[nq, :, :, :, :] / unitcells
                )
        #
        ...
        for n1 in range(big_nat):
            ...
            for n2 in range(big_nat):
                ...
                harm_normalp[n1 * 3 : (n1 + 1) * 3, n2 * 3 : (n2 + 1) * 3, :] = H_Rp[
                    CODE2[uc_1, uc_2],
                    atoms_1 * 3 : (atoms_1 + 1) * 3,
                    atoms_2 * 3 : (atoms_2 + 1) * 3,
                    :,
                ]
                harm_normalpp[n1 * 3 : (n1 + 1) * 3, n2 * 3 : (n2 + 1) * 3, :, :] = (
                    H_Rpp[
                        CODE2[uc_1, uc_2],
                        atoms_1 * 3 : (atoms_1 + 1) * 3,
                        atoms_2 * 3 : (atoms_2 + 1) * 3,
                ...
                if DIST < dist_lim:
                    for d1 in range(3):
                        for d2 in range(3):
                            ...
                            forces[a1, d1] += (
                                2.0 * 0.5 * dyn[a1 * 3 + d1, a2 * 3 + d2] * dA[d2]
                            )
                            stress[d1, d2] += (
                                -0.5
                                * dyn[a1 * 3 + d1, a2 * 3 + d2]
            ...
            for a in range(self.nat):
                for i in range(3):
                    st += (
                        ...
                        + str(a + 1)
                        + ")= "
                        + str(self.zstar[a][i, 0])
                        + " "
                        + str(self.zstar[a][i, 1])
                        + " "
                        + str(self.zstar[a][i, 2])
                        + "\n"
                    )
                st += "\n"
        st += "&end\n"
    ...
    def supercell_fourier_make_force_constants_derivative_harm(
        self, A, coords, low_memory=False
    ):
        ...
        hk, hk2, nonan, npa, npp, hkp, hkpp = self.get_hk_with_derivatives(
            [0, 0, 0], self.pos_crys
        )
        ...
        for nq in range(unitcells):
            ...
            counter[:] = [
                nq / (self.supercell[1] * self.supercell[2]),
                (nq / self.supercell[2]) % self.supercell[1],
                nq % (self.supercell[2]),
            ]
        ...
        for nr in range(unitcells):
            counter[:] = [
                nr / (self.supercell[1] * self.supercell[2]),
                (nr / self.supercell[2]) % self.supercell[1],
                nr % (self.supercell[2]),
            ]
